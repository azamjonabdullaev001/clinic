<template>
  <div class="min-h-screen bg-surface flex items-center justify-center px-4 relative overflow-hidden">
    ...
    <div class="absolute top-1/3 -left-32 w-96 h-96 bg-brand-200/15 rounded-full blur-[100px] animate-pulse-soft"></div>
    <div class="absolute bottom-1/4 -right-32 w-72 h-72 bg-brand-100/15 rounded-full blur-[80px] animate-pulse-soft" style="animation-delay: 2s"></div>
    ...
    <div class="relative w-full max-w-md">
      ...
      <div class="bg-white/90 backdrop-blur-xl rounded-3xl shadow-xl shadow-stone-200/40 p-8 md:p-10 animate-fade-up" style="animation-delay: 0.1s; opacity: 0">
        <div class="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 class="text-xl font-serif text-stone-900">Вход</h1>
            <p class="text-xs text-stone-400">Войдите в свой аккаунт</p>
          </div>
        </div>
        ...
        <p class="mt-8 text-center text-stone-400 text-sm">
          Нет аккаунта?
          <router-link to="/register" class="text-brand-600 hover:text-brand-700 font-semibold transition-colors"> Зарегистрируйтесь</router-link>
        </p>
      </div>
    </div>
  </div>
